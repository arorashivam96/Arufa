...
**Model:** `gpt-5-mini` (FDEBench cost tier 0.9). `reasoning_effort=low`, `max_completion_tokens=2048`. Triage is classification with fuzzy category boundaries (BioAuth-vs-Threat, Hull-vs-Systems-Engineering hardware, Comm-vs-Software) — a few reasoning tokens meaningfully improve the category / priority / escalation triple. Minimal reasoning was tried at v061 and lost ~3–4 pp Resolution locally with only marginal latency gain, so `low` is the shipped setting.

**Architecture:** single LLM call reasoning across all seven output fields at once. An earlier M13 iteration split classification into three parallel heads (category+team+escalation, priority, missing-info); it improved local N=50 scores but regressed the hidden T1 Resolution from 43.1 → 27.5 because the heads lost cross-dimension coherence. M14 reverted to the single-call M12 shape and kept only the deterministic post-processing (below).

**Prompt:** [`prompts/triage_system.md`](../py/apps/arufa/prompts/triage_system.md). Structured as: golden rules (don't trust "urgent" tone; quiet emergencies hide behind polite text; hull/atmosphere/zone always escalate) → 8 categories with 1-line descriptions → 7 teams with ownership hints → P1–P4 rubric → `needs_escalation` rule → walk-the-16-table pattern for `missing_information` ("emit only when concept is *absent* from the description; empty list is valid") → 7 worked examples covering the priority spectrum and NAMS → output JSON schema.
...
**Deterministic post-processing (runs after LLM parse, before the response envelope):**

1. **Routing clamp** — `arufa.triage.routing.team_for_category(category)` overrides the LLM's `assigned_team` with the canonical partner of the LLM's category choice. The label spec is 1:1, so the LLM's team pick is redundant and adds noise on the ~10 % of items where it disagrees with its own category.
2. **NAMS → empty missing_info** — when `category == "Not a Mission Signal"`, `missing_information` is forced to `[]`. Matches the observed gold pattern (NAMS items never have expected missing info) and prevents the model over-emitting concepts on spam / autoresponder tickets.
3. **Missing-info dedupe** — preserves first-seen order while dropping duplicate keys.
4. **Safety-rules catch-net** — [`triage/safety_rules.py`](../py/apps/arufa/arufa/triage/safety_rules.py). Regex triggers on `subject + description` for hull breach, atmosphere / life-support compromise, and restricted-zone access. When any trigger fires we **force** `needs_escalation=true` and `priority=P1` (category and team stay LLM-decided — forcing them risks worse F1 without helping escalation). This is the catch-net for the "quiet emergency" pattern where a polite senior-officer report might get down-ranked by the LLM. The generic `P1 ⇒ needs_escalation=true` invariant is enforced by the same module.
...
**Model:** `gpt-5-mini` (FDEBench cost tier 0.9). Chose mini over nano for the vision task because ~36% of the eval set is photographed / handwritten / degraded — the accuracy delta on that subset pays back the ~4 efficiency-score points of cost tier drop. `reasoning_effort=minimal`, `max_completion_tokens=4096` — extraction is a deterministic OCR + schema-map task, not a reasoning task. At `reasoning=low` the vision model was spending 4–8 k reasoning tokens per document, pushing P95 to ~24 s (worst=19 s → latency score clamped to 0). Minimal reasoning still runs the vision pass at full detail and cut local P95 to ~15 s (crossing the worst threshold), and lifted the `concurrent_burst` probe from FAIL → PASS (API resilience 71.4 → 100).
...
**Tool client and executor:** [`orchestrate/tool_client.py`](../py/apps/arufa/arufa/orchestrate/tool_client.py). `httpx.AsyncClient`, 5 s per-call timeout, one retry on 5xx with backoff, no retry on 4xx (records `skip_reason`). **Never raises** — returns `ToolCallResult(success=False, error=...)` so the workflow can continue and the response can report partial progress. The executor batches *consecutive same-tool steps* and fires each batch via `asyncio.gather` — the common T3 shape "search once, then act on each returned entity" gets parallelised on the act pass without a formal dependency graph. Different-tool boundaries stay sequential so `crm_search → send_email` still respects order. Full plan-wide parallelism was tested at v063 and traded −1.9 pp Resolution for a P95 improvement that still fell short of the 8 s worst threshold, so we reverted to same-tool batching. The state module wraps outcomes in `StepExecuted` entries with truncated result summaries.
